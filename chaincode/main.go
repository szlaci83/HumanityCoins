...
// HumanityChaincode is the receiver for chaincode functions
	type HumanityChaincode struct{
		...
		// function to initialize chaincode add entities and points to start with to the ledger.
func (t *HumanityChaincode) addUser(stub shim.ChaincodeStubInterface, args []string) (string, error){
	...
	for index := 0; index < len(args); index += 2{
  	...
  	if err != nil || entityJson == nil {
   		return "",fmt.Errorf("Converting entity struct to entityJSON failed")
		}

  	// write entity attributes into ledger
  	err = stub.PutState(userID, entityJson)
  	if err != nil {
  		return "", fmt.Errorf("Error: could not update ledger")
	  }
	}
	...
	// Notify listeners that an event  have been executed (check line 19 in the file invoke.go)
	...
	func (t *HumanityChaincode) addThanks(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	  ...
	  thankObj := thank{}
	  thankObj.Thanker = args[1]
	  thankObj.ThankType = args[2]
	  thankObj.Message = args[3]
    ...
    if err != nil || entityJSON == nil {
	  ...
	  // Notify listeners that an event  have been executed (check line 19 in the file invoke.go)
		err = stub.SetEvent("eventAddThanks", []byte{})
		if err != nil {
			return "",fmt.Errorf("Writing updated entity to ledger failed")
		}
    ...
    fn, args := stub.GetFunctionAndParameters()
	  if fn == "addThanks" {
       ...
       result, err = t.addUser(stub, args)
    } else {
